...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_detalle_factura_uuid') THEN
    ALTER TABLE detalle_facturas ADD CONSTRAINT fk_detalle_factura_uuid
      FOREIGN KEY (factura_uuid) REFERENCES facturas (uuid) ON UPDATE CASCADE ON DELETE CASCADE;
  END IF;
END $$;